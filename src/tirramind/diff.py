...
def fill_exchange(events: pd.DataFrame, *, root: str = DEFAULT_ROOT) -> pd.DataFrame:
    ...
    from the ``exchange`` snapshots: for removals the latest capture at or
    before the step (what it was on); for new listings the earliest capture
    at or after (what it joined). Rows outside the captured range stay blank;
    ...
    hx = hist.sort_values("at_ts")[["cik", "ticker", "exchange", "at_ts"]].rename(columns={"exchange": "ex_hist"})
    blank = ev.exchange.eq("")
    for mask, col, direction in ((blank & ev.event.ne("LISTED"), "from_captured_at", "backward"),
                                 (blank & ev.event.eq("LISTED"), "to_captured_at", "forward")):
        sub = ev[mask].copy()
        if sub.empty:
            continue
        sub["ev_ts"] = pd.to_datetime(sub[col], utc=True)
        sub = sub.sort_values("ev_ts")
        got = pd.merge_asof(sub, hx, left_on="ev_ts", right_on="at_ts", by=["cik", "ticker"], direction=direction)
