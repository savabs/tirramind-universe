...
Built 2026-09-11T22:43:03+00:00 by tirramind 0.1.0. Derived from SEC public
...
| file | rows | what |
|---|---|---|
| `tickers_latest.parquet` | 10426 | current (cik, ticker, name, exchange) |
| `events.parquet` | 104076 | every change between consecutive snapshots |
| `delistings.parquet` | 14316 | permanent removals (not re-listed, not from a suspect capture) with a cause and evidence accessions |
| `filings.parquet` | 78713 | Form 25 / 15 and item-filtered 8-K filings, 2015→ |

## Events (270 snapshots, 2017-08-28 → 2026-09-11)
| event | count |
|---|---|
| DELISTED_FROM_MAP | 43754 |
| EXCHANGE_CHANGED | 2 |
| LISTED | 47925 |
| NAME_CHANGED | 8999 |
| SYMBOL_CHANGED | 3396 |
...
## What a "removal" is
`events.parquet` has 43754 raw `DELISTED_FROM_MAP` rows. **50% of
...
Of the removals that are neither re-listed nor suspect (14316),
...
not delistings (SYMBOL_CHANGED 1423, EXCHANGE_TRANSFER 236, SUCCESSION 190).
They stay in `delistings.parquet` with that cause; the shares below are
over the remaining 12467 true delistings. The raw rows stay in `events.parquet`; nothing is deleted.
...
|---|---|---|---|---|---|---|---|---|
| (blank) | 62 | 25 | 117 | 29 | 1106 | 6 | 1345 | 18% |
| CBOE | 0 | 0 | 19 | 0 | 12 | 0 | 31 | 61% |
| NAS | 0 | 1 | 0 | 0 | 0 | 0 | 1 | 100% |
| NYSE | 33 | 4 | 1029 | 536 | 109 | 12 | 1723 | 94% |
| Nasdaq | 103 | 8 | 1399 | 1180 | 207 | 38 | 2935 | 93% |
| OTC | 153 | 159 | 253 | 105 | 2636 | 16 | 3322 | 21% |
